...
function runSpawn(event) {
    return new Promise((resolve, reject) => {
        ...
        Touch()
            ...
            .then(() => FFmpeg())
            .then(() => {
                const type = WROOMID.includes(Number(roomid)) ? 'Whitelist' : (BROOMID.includes(Number(roomid)) ? 'Blacklist' : (CROOMID.includes(Number(roomid)) ? 'Code' : 'runSpawn: Other'))
                console.log('runSpawn: ' + type)
                switch (type) {
                    case "Whitelist":
                    ...
                    case "Blacklist":
                        ...
                        rcloneB.on('error', error => console.log(`[rclone-error ] (${EventId}): ${error}`));
                        break;
                    case "Code":
